...
# To establish the connection with MySQL server on 100.70.99.38 (My IP address) and store the status and
# connection status in the vars.status and vars.condition
def estConnect():
    try:

        # Connecting to the MySQL server
        global_.conn = sql.connect(host="100.70.99.38", user=global_.username, password=global_.password)
